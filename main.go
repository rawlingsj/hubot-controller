...
import (
	...
	buildclient "github.com/rawlingsj/hubot-controller/client"
	"k8s.io/api/core/v1"
...
func main() {
	...
	indexer, informer := cache.NewIndexerInformer(buildListWatcher, &build.Build{}, 0, cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			notify(obj.(*build.Build).GetName())
		},
	}, cache.Indexers{})
...
func notify(name string) {
	...
	room := os.Getenv("HUBOT_CONTROLLER_ROOM")
	if room == "" {
		room = "default"
	}
	requestUrl := fmt.Sprintf("http://hubot/hubot/notify/%s", room)
	data := url.Values{}
	data.Set("message", fmt.Sprintf("%s has started", name))
